...
const UserProfile: React.FC<UserProfileProps> = ({ user, onLogout, onUpdateUser }) => {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      alert("Perfil Elite actualizado globalmente.");
    } catch (err) {
      ...
      <header className="flex flex-col items-center text-center space-y-8 p-10 md:p-14 bg-white dark:bg-darkCard rounded-[50px] md:rounded-[60px] luxury-shadow border border-quartz/10 relative overflow-hidden">
        ...
        <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
           <div className="w-44 h-44 rounded-[50px] border-4 border-offwhite dark:border-onyx overflow-hidden shadow-2xl relative transition-all active:scale-95 group-hover:brightness-90">
              ...
              <div className="absolute inset-0 bg-onyx/40 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center transition-all text-white">
                 <Icons.Plus />
                 <span className="text-[10px] font-black uppercase tracking-widest mt-3">Mudar Foto</span>
              </div>
           </div>
        ...
        <div className="space-y-3 w-full">
           ...
           <div className="flex justify-center">
              <span className="bg-gold/10 text-gold px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest border border-gold/10">Patente Verificada</span>
           </div>
        </div>
      </header>
      ...
      <div className="bg-white dark:bg-darkCard rounded-[50px] md:rounded-[60px] p-8 md:p-16 luxury-shadow border border-quartz/10 space-y-12">
         <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-4">
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome Completo" className="w-full bg-offwhite dark:bg-onyx rounded-full py-5 px-8 outline-none font-bold dark:text-white transition-all shadow-inner" />
            </div>
            <div className="space-y-4">
              <label className="text-[10px] font-black uppercase tracking-[0.4em] text-ruby ml-1">Contacto VIP</label>
              <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Nº de Telemóvel" className="w-full bg-offwhite dark:bg-onyx rounded-full py-5 px-8 outline-none font-bold dark:text-white transition-all shadow-inner" />
            </div>
         </div>

         <div className="space-y-8">
            <div className="text-center">
               <h3 className="text-3xl font-serif font-black dark:text-white italic">Onde você <span className="text-ruby">Atua.</span></h3>
            </div>
            <div className="relative h-[450px] w-full rounded-[45px] overflow-hidden border border-quartz/10 luxury-shadow">
         ...
         <div className="flex flex-col gap-5 pt-6 max-w-xl mx-auto">
            <button onClick={handleSaveProfile} disabled={isSaving} className="w-full py-7 bg-ruby text-white rounded-full font-black uppercase text-[12px] tracking-[0.5em] shadow-xl active:scale-95 transition-all flex items-center justify-center gap-4">
