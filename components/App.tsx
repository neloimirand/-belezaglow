...
const App: React.FC = () => {
  const [showLanding, setShowLanding] = useState(() => {
    return !localStorage.getItem('glow_user');
  });
  ...
  const handleTabChange = useCallback((tab: string) => {
    setActiveTab(tab);
    // Limpa modais e detalhes ao navegar para qualquer aba principal, garantindo foco total no conteúdo da aba
    setSelectedProvider(null);
    setIsBookingModalOpen(false);
    setIsChatOpen(false);
    
    // Se clicar em Home, garante que o scroll volte ao topo
    if (tab === 'home') {
  ...
  return (
    <div className="min-h-screen bg-offwhite dark:bg-onyx">
      ...
      >
        {activeTab === 'home' && (
          <Home 
            onStartExploring={handleTabChange} 
        ...
        {activeTab === 'profile' && <UserProfile user={displayUser} onLogout={handleLogout} onNavigateToSupport={() => handleTabChange('support')} />}
        ...
        {/* FLUXO DE DETALHES - Agora corretamente gerenciado pelo state central */}
        ...
        {selectedProvider && isBookingModalOpen && (
          <BookingModal 
            ...
            onSuccess={(date, time) => {
              ...
              notify("Agendamento Confirmado", `Horário reservado para ${date} às ${time}.`, "success");
            }} 
          />
        )}
      </Layout>
